...
export const useLatestBlock = (): BlockData => {
  ...
  useEffect(() => {
    const networkName = import.meta.env.PROD ? 'mainnet' : 'sepolia';
    const provider = new AlchemyProvider(networkName, import.meta.env.VITE_ALCHEMY_APP_KEY);
    ...
    const ws = new WebSocket(`wss://eth-${networkName}.g.alchemy.com/v2/${import.meta.env.VITE_ALCHEMY_APP_KEY}`);
